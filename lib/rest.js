...
var Rest = function(baseUrl, opts)
{
   ...
   */
  this.baseUrl = baseUrl || (window ? window.location.href.split('#')[0] : 'http://localhost');
 ...
 * @name Rest#addFilter
 * @param {String} type - type of the filter. Can be one of: 'url', 'params', 'options', 'data'
...
Rest.prototype.request = function(/*method, path, [options], callback*/)
{
  ...
  var requestOpts = copyOptions(this.defaultRequestOptions);
  ...
  // apply data filter
  request.data = this.executeFilters('requestData', copyOptions(request.data), request);

  // apply params filters
  request.params = this.executeFilters('params', copyOptions(request.params), request);
  ...
  request.headers = this.executeFilters('requestHeaders', copyOptions(request.headers), request);

  var cachedResponse;
  if (request.method === 'GET' && this.cacheLifetime > 0) {
