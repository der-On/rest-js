...
var request = (typeof window !== 'undefined') ? require('browser-request') : require('request');
...
RestRequest.prototype.prepare = function()
{
  ...
  switch(this.dataType) {
    case 'json': this.headers['accept'] = 'application/json';
      break;
  }
};
...
RestRequest.prototype.send = function(callback)
{
  ...
  if (typeof window !== 'undefined') {
    if (window.jQuery) {
      ...
      this._sendBrowserRequest(callback);
    }
  }
  else {
...
RestRequest.prototype._sendBrowserRequest = function(callback)
{
  var self = this;
  ...
  function onResponse(err, xhr, body)
  {
    var statusCode = xhr.status;
    var headers = xhr.getAllResponseHeaders();
    var data = body || xhr.responseText || xhr.responseXml;
    if (err) {
      err.statusCode = statusCode;
    }
    var response = new RestResponse(self, statusCode, headers, data, err);
